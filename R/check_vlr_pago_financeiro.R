#' @export
check_vlr_pago_financeiro <- function(detalhe, totais, stop_on_failure = FALSE, output = FALSE) {
  key <- c("ano", "mes_cod")
  ...
  x <- detalhe |> 
    checksplanejamento:::aggregate(
      "vlr_detalhe", 
      by = key, 
      rename = list(vlr_pago_financeiro = "vlr_detalhe")
    )
  ...
  y <- totais |> 
    checksplanejamento:::aggregate(
      "vlr_totais", 
      by = key, 
      rename = list(vlr_pago_financeiro = "vlr_totais")
    )
