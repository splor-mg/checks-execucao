#' @export
check_vlr_pago_orcamentario <- function(detalhe, totais, stop_on_failure = FALSE, output = FALSE) {
  key <- c("ano", "mes_cod")
  ...
  x <- detalhe |> 
    checksplanejamento:::aggregate(
      "vlr_detalhe", 
      by = key, 
      rename = list(vlr_pago_orcamentario = "vlr_detalhe")
    )
  ...
  y <- totais |> 
    checksplanejamento:::aggregate(
      "vlr_totais", 
      by = key, 
      rename = list(vlr_pago_orcamentario = "vlr_totais")
    )
